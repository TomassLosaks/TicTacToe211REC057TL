...
class Vh(parent: ViewGroup, onClick: (row: Int, col: Int) -> Unit) : RecyclerView.ViewHolder(
    ...
    fun bind(cell: Boolean?, row: Int, col: Int) {
        ...
        when (cell) {
            true -> {
                markTv.setTextColor(Color.parseColor("#2700A6")) // X — красный
            }
            false -> {
                markTv.setTextColor(Color.parseColor("#00A600")) // O — синий
            }
            null -> {
                markTv.setTextColor(Color.parseColor("#FFFFFF")) // пустая — тёмно-серая
            }
        }
    }
}